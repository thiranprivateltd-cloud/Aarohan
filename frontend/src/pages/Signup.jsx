...
export const Signup = () => {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      const mockGooglePayload = {
        email: formData.email || `google.user_${Date.now()}@gmail.com`,
        name: formData.name || 'Google Learner',
  ...
  return (
    <main role="main" className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-md border border-amber-200/60 mt-10 font-lexend">
      <h2 className="text-2xl font-bold text-slate-900 mb-6 text-center">Create your Aarohan Account</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="signup-name" className="block text-sm font-bold text-slate-900 mb-1">Full Name</label>
          <input
            id="signup-name"
            type="text"
            name="name"
        ...
        <div>
          <label htmlFor="signup-email" className="block text-sm font-bold text-slate-900 mb-1">Email Address</label>
          <input
            id="signup-email"
            type="email"
            name="email"
        ...
        <div>
          <label htmlFor="signup-password" className="block text-sm font-bold text-slate-900 mb-1">Password</label>
          <input
            id="signup-password"
            type="password"
            name="password"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="signup-role" className="block text-sm font-bold text-slate-900 mb-1">Role</label>
            <select
              id="signup-role"
              name="role"
              value={formData.role}
              onChange={handleChange}
              className="w-full px-3 py-2 border border-amber-300 rounded-md text-sm text-slate-900 bg-amber-50/40 font-bold"
            >
          ...
          <div>
            <label htmlFor="signup-lang" className="block text-sm font-bold text-slate-900 mb-1">Preferred Language</label>
            <select
              id="signup-lang"
              name="language"
      ...
      <p className="text-center text-sm text-slate-700 mt-4 font-medium">
        Already have an account? <Link to="/login" className="text-[#C4623A] font-bold hover:underline">Log in</Link>
      </p>
    </main>
  );
};
